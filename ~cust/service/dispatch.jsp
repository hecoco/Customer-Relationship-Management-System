...
function confirmDelete(id)
{
  if(confirm('是否删除?'))
  {
  location.href="service!delete?cs.svrId="+id;
  }
...
function fy(page){
		...
		document.getElementById("count1").value=document.getElementById("count").value;
		document.getElementById("form1").submit();
	}
	function go(){
		document.getElementById("page").value=document.getElementById("page1").value;
		document.getElementById("count1").value=document.getElementById("count").value;
		document.getElementById("form1").submit();
	}
</script>
...
<body>
	...
	<input type="hidden" name="page" id="page"/>
		<table class="query_form_table" height="53">
			<tr>
				<th height="28">客户</th>
				<td><input name="cs.svrCustName" value="<s:property value='cs.svrCustName' />" />
				</td>
				<th height="28">概要</th>
				<td><input name="cs.svrTitle"  value="<s:property value='cs.svrTitle' />" />
				</td>
	...
	<table class="data_list_table">
		...
		<s:iterator value="#request.list">
			<form id="form1" action="service!update2" method="post">
				<%-- <s:if test="svrStatus=='新创建'"> --%>
					<tr>
						...
						<input type="hidden" name="cs.svrId"
							value="<s:property value='svrId'/>" />

						<td class="list_data_text"><select name="cs.svrDueId">
								<option>请选择...</option>

								<s:iterator value="#session.user">
									<s:if test="role.roleId==3 ">
										<option value="<s:property value='usrId'/>">
											<s:property value="usrName" />
										</option>
										<input type="hidden" name="cs.svrDueTo"
											value="<s:property value='usrName'/>" />
									</s:if>

								</s:iterator>
						</select>
							<button class="common_button">分配</button></td>

						<td class="list_data_op"><img
							onclick="javascript:confirmDelete(<s:property value="svrId"/>)"
							title="删除" src="html/images/bt_del.gif" class="op_button" /></td>
					</tr>
			<%-- 	</s:if> --%>
			</form>
		</s:iterator>
